...
class DistrLearnerWta(nn.Module):
    def __init__(self, model_info, params=False, fitparams=False,
                 start_params=None):
        super(DistrLearnerWta, self).__init__()
    ...
    def forward(self, x):
        ...
        # WTA - get max act
        _, ind = torch.max(act, 0)
        self.wta_mask[:] = False  # clear
        self.wta_mask[:, ind] = True
...
def train_distr_wta(model, inputs, output, n_epochs, shuffle=False):
