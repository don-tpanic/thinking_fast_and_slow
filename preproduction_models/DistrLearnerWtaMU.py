...
class DistrLearnerWtaMU(nn.Module):
    def __init__(self, model_info, params=False, fitparams=False,
                 start_params=None):
        super(DistrLearnerWtaMU, self).__init__()
        ...
        # wta mask: 2d for fc1 weights (2 outputs). index 1st dim [0] for units
        self.wta_mask = torch.zeros([self.nn_sizes[1], self.max_nunits],
                                    dtype=torch.bool)
    ...
    def forward(self, x):
        ...
        units_output = act * self.active_units * self.wta_mask[0]  # 1D only
...
def train_distr_wta_mu(model, inputs, output, n_epochs, shuffle=False):
    ...
    for epoch in range(n_epochs):
        ...
        for x, target in zip(inputs_, output_):
            ...
            # k-WTA mask computed here coz if recruit (below), mightn't be top
            mvn1 = torch.distributions.MultivariateNormal(
                model.units, scale_tril=torch.tril(model.attn))
            act = torch.exp(mvn1.log_prob(x))
            _, ind = (
                torch.topk(act, int(model.max_nunits * model.params['k'])))
            model.wta_mask[:] = False  # clear
            model.wta_mask[:, ind] = True
            ...
            with torch.no_grad():
                model.fc1.weight.grad.mul_(model.mask1 * model.wta_mask)
            ...
            if recruit and any(model.active_units == 0):  # in case recruit too many
                ...
                # for k-WTA, only replace mispredicted units
                mispred_units = (
                    torch.argmax(model.fc1.weight[:, ind].detach(),
                                 dim=0) != target
                    )
                ...
                # k-WTA mask computed here coz if recruit, might not be top
                model.wta_mask[:] = False  # clear
                model.wta_mask[:, recruit_ind] = True
                # # also include previously correct units? not nec
                # model.wta_mask[:, ~mispred_units] = True
