...
import models
from train_investigate_config126 import fit
...
def train_model(config_version):
    ...
    investigation = np.empty((num_runs, num_blocks, 100, 11))
        ...
        for epoch in range(num_blocks):
            ...
            for i in range(len(shuffled_dataset)):
                ...
                model, \
                    ...
                    y_logits_fast, y_logits_slow, y_logits_total, y_true, \
                        item_proberror_fast, item_proberror_slow, item_proberror_total = \
                        fit(
                            model=model, 
                            x=x,
                ...
                investigation[run, epoch, i, :2] = y_logits_fast.detach().numpy()
                investigation[run, epoch, i, 2:4] = y_logits_slow.detach().numpy()
                investigation[run, epoch, i, 4:6] = y_logits_total.detach().numpy()
                investigation[run, epoch, i, 6:8] = y_true
                investigation[run, epoch, i, 8] = item_proberror_fast
                investigation[run, epoch, i, 9] = item_proberror_slow
                investigation[run, epoch, i, 10] = item_proberror_total
    ...
    np.save(os.path.join(results_path, f'investigation.npy'), investigation)
